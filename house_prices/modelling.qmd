...
### The 12 predictors correlated with the target
...
resampling_comparison <-
  map2(mod_resamples, mod_names, ~ best_rmse2(res_obj = .x, model_name = .y)) %>%
...
```{r}
resamples_file <- "data/xgb_base_log_bor_res_wl_cv10_new_split.rds"
...
resamples_file <- "data/lgb_base_log_bor_res_wl_cv10_new_split_noknn.rds"
...
xgb_wf <- workflow() %>% 
  add_model(xgb_spec) %>% 
  add_recipe(base_log_boruta_recipe) # the recipe here should be the one that was used for this model
...
```{r, eval=F}
write_csv(pred_submission_df, "data/prediction_submission_lgb_log_bor_wl_noknn.csv", quote = "none")
```
