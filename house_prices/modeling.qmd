...
```{r}
resamples_file <- "data/lrl_decorr_res_.rds"
...
```{r}
resamples_file <- "data/lrl_pca_res_cv5.rds"
...
if (file.exists(resamples_file)) {
  ...
  saveRDS(lrl_pca_res, resamples_file)
}
...
pred_submission_df <- tibble(Id = data_submission$Id, SalePrice = prediction_data_submission$.pred)
pred_submission_df
```

```{r}
write_csv(pred_submission_df, "data/prediction_submission.csv", quote = "none")
```
