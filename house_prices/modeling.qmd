...
22 PCs explaining 0.8 of the variance

```{r, fig.width=6, fig.height=3}
var_df %>%
  filter(var_cum_sum < .80) %>% 
...
# v=5 lead to lower numbers in training compared to v=10
# but v=10 has lower numbers in testing (overfitting!)
...
```{r}
resamples_file <- "data/lrl_decorr_res.rds"
...
if (file.exists(resamples_file)) {
  ...
  set.seed(133)
  lrl_mod <- linear_reg(penalty = tune(),
                       mixture = 1) %>%
  ...
  saveRDS(lrl_decorr_res, "data/lrl_decorr_res.rds")
}
...
```{r}
resamples_file <- "data/lrl_pca_res.rds"

if (file.exists(resamples_file)) {
  print("Found resamples file")
  lrl_pca_res <- readRDS(resamples_file)
} else {
  lr_pca_wf <- workflow() %>%
    add_model(lr_mod) %>%
  ...
  saveRDS(lr_pca_res, resamples_file)
}

autoplot(lrl_pca_res)
```

#### Best models

```{r}
show_best(lrl_pca_res)
```
...
```{r}
resamples_file <- "data/rf_base_res_cv5.rds"
...
```{r}
resamples_file <- "data/mlp_pca_res_cv5.rds"

if (file.exists(resamples_file)) {
  print("Found resamples file")
  mlp_pca_res <- readRDS(resamples_file)
} else {
  mlp_mod <-
  ...
  saveRDS(mlp_pca_res, resamples_file)
}

autoplot(mlp_pca_res)
```

```{r}
show_best(mlp_pca_res)
```
...
```{r}
resamples_file <- "data/mlp_decorq_res_cv10.rds"

if (file.exists(resamples_file)) {
  print("Found resamples file")
  mlp_decorq_res <- readRDS(resamples_file)
} else {
  ...
  saveRDS(mlp_decorq_res, resamples_file)
}
...
# best model
best_mod <- lrl_decorr_res %>% 
  select_best("rmse")

# use hyper parameters of the best model
last_mod <-
  linear_reg(penalty = best_mod$penalty,) %>%
...
data_submission_processed <- final_recipe %>%
