...
```{r}
df %>% filter(is.na(bmi)) %>% group_by(stroke) %>% summarise(n.bmi=n())
```
...
BMI is now complete, in total approx. 2000 observations are gone
...
2. Predictive mean matching (`pmm`) for `bmi` (if no trimming were made)
